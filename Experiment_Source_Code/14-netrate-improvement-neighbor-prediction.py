...
# rewrite prediction file
with open('prediction-neighbors-probs-exp.txt','w') as writer:
    None
...
with open('prediction-neighbors-experiment-nc.txt','r') as predFile:
    for pred in predFile:
        ...
        predArr.append({'target_node': predJson['target_node'],'neighbors':predJson['neighbors'],'rand_neighbors':predJson['rand_neighbors']})
...
for sample in predArr:
    ...
    rand_neighbors = sample['rand_neighbors'] 
    ...
    randNeighbors = []
    for myneighbor in rand_neighbors:
        if myneighbor not in convexNodes:
            # [observationCascadesId.append(x) for x in nodeCascades[myneighbor]]
            # check intersection with parent cascades
            convexNodes[myneighbor] = convexNodesCount
    ...
    if num_nodes > 0:
        ...
        with open('prediction-neighbors-probs-exp.txt','a') as writer:
