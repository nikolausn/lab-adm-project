...
"""
# Using smaple from the result of experiment 12
predArr = []
with open('prediction-neighbors-probs-20170426.txt','r') as predFile:
    for pred in predFile:
        predJson = json.loads(pred)
        predArr.append({'target_node': predJson['target_node'],'neighbors':predJson['neighbors']})
...
for sample in predArr:
    ...
    if num_nodes > 0:
        ...
        with open('prediction-neighbors.txt','a') as writer:
            writer.write(json.dumps({'target_node': target_node,'alpha': Aarray.T.tolist(), 'parents': edges[target_node],'neighbors': neighborArr,'trans':convexNodesArr, 'total_nodes':len(convexNodesArr), 'totalcas': len(observationCascades.keys())})+'\n')
