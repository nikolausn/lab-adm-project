...
rows = c.execute("SELECT docid,sum(total) as total_feature FROM exfeature group by docid order by total_feature desc")
...
docsFile = open('docs-obs.txt','a')
for row in rows:
	print(row[0])
	c2 = conn.cursor()
	rows2 = c2.execute("SELECT b.word,c.doc,a.total from exfeature a,words b,docs c where a.docid=c.docid and b.wordid=a.wordid and a.docid=?",[row[0]])
	firstRow = True
	for row2 in rows2:
		if firstRow:
