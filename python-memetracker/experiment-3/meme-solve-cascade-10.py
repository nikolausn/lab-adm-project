...
for target_node in nodeCasSort:
    ...
    if num_nodes > 0:
        ...
        for x in range(len(tempA)):
            if convexNodesArr[x] in edges[target_node] or convexNodesArr[x] == target_node:
                #Atwo[convexNodesArr[x]] = A[:,convexNodes[target_node]][x]
                Atwo[convexNodesArr[x]] = tempA[x]
            else:
                Athree[convexNodesArr[x]] = { 'from': derivedFrom[convexNodesArr[x]],'val': tempA[x]}

        with open('prediction-child-neighbor.txt','a') as writer:
            writer.write(json.dumps({'target_node': target_node,'alpha_parent': Atwo,'alpha_neighbor': Athree})+'\n')
