...
# nodes
try:
	c.execute("CREATE TABLE cluster (cid INTEGER,text)")
	c.execute("CREATE TABLE clusterurl (cid INTEGER,date,type,domain,url TEXT)")
...
def insertCluster(cluster):
	...
	c = conn.cursor()
	c.execute("INSERT INTO cluster VALUES (?,?)",params)
...
def insertClusterUrl(url):
	...
	c = conn.cursor()
	c.execute("INSERT INTO clusterurl VALUES (?,?,?,?,?)",params)
...
with open('clust-qt08080902w3mfq5.txt','r') as memes:
	for meme in memes:
		...
		if(len(memeSplit)==5):
			insertCluster({'cid': cid,'text': memeSplit[2]})
			cid+=1
